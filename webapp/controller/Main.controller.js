	...
	return Controller.extend("org.fater.clustermanagement.controller.Main", {
		...
		confirmAddCompany: function() {
			...
			if (!companySelect.getSelectedKey()) {
				...
				MessageToast.show(this.getTranslation("checkFieldsError"), {
					"duration": 6000
				});
				return;
			}
		...
		handleRemoveCompany: function(oEvent) {
			...
			var cluster;
			if (clusterComSet.length > 1) {
				cluster = clusterComSet[indexCompany - 1];
				this.getView().getModel("oUtilsModel").setProperty("/selectedTabKey", cluster.ClusterId + "_" + cluster.Bukrs);
			} else if (clusterComSet.length === 1) {
				cluster = clusterComSet[0];
				this.getView().getModel("oUtilsModel").setProperty("/selectedTabKey", cluster.ClusterId + "_" + cluster.Bukrs);
			} else {
				var clusterPath = sPath.substr(0, sPath.lastIndexOf("/"));
				clusterPath = clusterPath.substr(0, clusterPath.lastIndexOf("/"));
				clusterPath = clusterPath.substr(0, clusterPath.lastIndexOf("/"));
				var index = clusterPath.substr(clusterPath.lastIndexOf("/") + 1);
				this.getView().byId("clusterList").getItems()[index].getContent()[0].setExpanded(false);
			}
		},
